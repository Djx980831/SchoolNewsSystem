 ...
 *
 */
@WebServlet("/showType1News")
public class ShowType1News extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<News> type1newes=service.showNews(pageNum, type);
		
		News news=new News();
		Iterator<News> it=type1newes.iterator();
/*		while(it.hasNext()) {
		...
		session.setAttribute("type1newes", type1newes);
		session.setAttribute("page1", pageNum);
		
		if(flag!=null) {
			
			response.sendRedirect(request.getContextPath()+"/newsDetail.jsp");
